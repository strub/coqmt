...
let ex_to_subst = fun subst ->
  (subst.us_sigma, subst.us_metasubst, subst.us_evarsubst)

let ex_from_subst = fun (sigma, metasubst, evarsubst) delayed ->
  { us_sigma     = sigma;
...
let unify_0_with_initial_metas ((top_sigma,ms,es) as subst) conv_at_top env cv_pb flags m n =
  let rec unirec_rec curenvnb pb b flags substn curm curn =
    let cM = Evarutil.whd_head_evar substn.us_sigma curm
    and cN = Evarutil.whd_head_evar substn.us_sigma curn in
      ...
      if flags.can_dp_delayed then begin
        if (delayable env cM) && (delayable env cN) then
          ...
          in ex_delayed delayed substn
        else
          unirec_rec_struct curenvnb pb b flags substn cM cN
      end else
        unirec_rec_struct curenvnb pb b flags substn cM cN

  and unirec_rec_struct (curenv,nb as curenvnb) pb b flags substn cM cN =
      match (kind_of_term cM,kind_of_term cN) with
	| Meta k1, Meta k2 ->
	    let stM,stN = extract_instance_status pb in
	    if k2 < k1
	    then ex_metasubst (k1,cN,stN) substn
	    else if k1 = k2 then substn
	    else ex_metasubst (k2,cM,stM) substn
	| Meta k, _ when not (dependent cM cN) ->
	    (* Here we check that [cN] does not contain any local variables *)
	    if nb = 0 then
        ex_metasubst (k,cN,snd (extract_instance_status pb)) substn
      else if noccur_between 1 nb cN then
        ex_metasubst (k,lift (-nb) cN,snd (extract_instance_status pb)) substn
	    else error_cannot_unify_local curenv substn.us_sigma (m,n,cN)
	| _, Meta k when not (dependent cN cM) ->
	    (* Here we check that [cM] does not contain any local variables *)
	    if nb = 0 then
        ex_metasubst (k,cM,fst (extract_instance_status pb)) substn
	    else if noccur_between 1 nb cM
	    then
        ex_metasubst (k,lift (-nb) cM,fst (extract_instance_status pb)) substn
	    else error_cannot_unify_local curenv substn.us_sigma (m,n,cM)
	| Evar ev, _ -> ex_evarsubst (ev,cN) substn
	| _, Evar ev -> ex_evarsubst (ev,cM) substn
	| Lambda (na,t1,c1), Lambda (_,t2,c2) ->
	    unirec_rec (push (na,t1) curenvnb) topconv true flags
	      (unirec_rec curenvnb topconv true flags substn t1 t2) c1 c2
	| Prod (na,t1,c1), Prod (_,t2,c2) ->
	    unirec_rec (push (na,t1) curenvnb) (prod_pb pb) true flags
	      (unirec_rec curenvnb topconv true flags substn t1 t2) c1 c2
	| LetIn (_,a,_,c), _ -> unirec_rec curenvnb pb b flags substn (subst1 a c) cN
	| _, LetIn (_,a,_,c) -> unirec_rec curenvnb pb b flags substn cM (subst1 a c)

	| Case (_,p1,c1,cl1), Case (_,p2,c2,cl2) ->
            array_fold_left2 (unirec_rec curenvnb topconv true flags)
	      (unirec_rec curenvnb topconv true flags
		  (unirec_rec curenvnb topconv true flags substn p1 p2) c1 c2) cl1 cl2
	...
	| App (f1,l1), _ when
	    ...
	    not (dependent f1 cN) ->
        ex_from_subst
          (solve_pattern_eqn_array curenvnb f1 l1 cN (ex_to_subst substn))
          substn.us_delayed
	...
	| _, App (f2,l2) when
	    ...
	    not (dependent f2 cM) ->
        ex_from_subst
	        (solve_pattern_eqn_array curenvnb f2 l2 cM (ex_to_subst substn))
          substn.us_delayed
	...
	| App (f1,l1), App (f2,l2) ->
	    ...
	    and len2 = Array.length l2 in
		  ...
		  array_fold_left2 (unirec_rec curenvnb topconv true flags)
		    (unirec_rec curenvnb pb true flags substn f1 f2) l1 l2
		with ex when precatchable_exception ex ->
		  try expand curenvnb pb b flags substn cM f1 l1 cN f2 l2
		  with ex when precatchable_exception ex ->
		    canonical_projections curenvnb pb b cM cN flags substn)

	| _ ->
	    try canonical_projections curenvnb pb b cM cN flags substn
	    with ex when precatchable_exception ex ->
		...
		let (f2,l2) =
		  ...
		  expand curenvnb pb b flags substn cM f1 l1 cN f2 l2

  and expand (curenv,_ as curenvnb) pb b flags substn cM f1 l1 cN f2 l2 =
    ...
    if
	 ...
	 essentially for apply)... *)
      ...
      | None -> false
      | Some convflags ->
      let subst = if flags.use_metas_eagerly then substn.us_metasubst else ms in
      ...
      | Some n1 ->
	  if is_trans_fconv (conv_pb_of pb) convflags env substn.us_sigma m1 n1
	  then true else
	    if is_ground_term substn.us_sigma m1 && is_ground_term substn.us_sigma n1 then
	      error_cannot_unify curenv substn.us_sigma (cM,cN)
	    else false
    then
      substn
    else
	...
	| None -> error_cannot_unify curenv substn.us_sigma (cM,cN)
	| Some true ->
	    (match expand_key curenv cf1 with
	    | Some c ->
		      unirec_rec curenvnb pb b flags substn
            (whd_betaiotazeta substn.us_sigma (mkApp(c,l1))) cN
	    | None ->
		(match expand_key curenv cf2 with
		| Some c ->
		    unirec_rec curenvnb pb b flags substn cM
          (whd_betaiotazeta substn.us_sigma (mkApp(c,l2)))
		| None ->
		    error_cannot_unify curenv substn.us_sigma (cM,cN)))
	| Some false ->
	    (match expand_key curenv cf2 with
  ...
  and canonical_projections curenvnb pb b cM cN flags substn =
    let f1 () =
      if isApp cM then
	let f1l1 = decompose_app cM in
	  if is_open_canonical_projection substn.us_sigma f1l1 then
	    let f2l2 = decompose_app cN in
	      solve_canonical_projection curenvnb pb b cM f1l1 cN f2l2 flags substn
	  else error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)
      else error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)
    in
      if flags.modulo_conv_on_closed_terms = None then
	error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)
      else
	try f1 () with e when precatchable_exception e ->
	  if isApp cN then
	    let f2l2 = decompose_app cN in
	      if is_open_canonical_projection substn.us_sigma f2l2 then
		let f1l1 = decompose_app cM in
		  solve_canonical_projection curenvnb pb b cN f2l2 cM f1l1 flags substn
	      else error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)
	  else error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)

  and solve_canonical_projection curenvnb pb b cM f1l1 cN f2l2 flags substn =
    let (c,bs,(params,params1),(us,us2),(ts,ts1),c1,(n,t2)) =
      try Evarconv.check_conv_record f1l1 f2l2
      with Not_found -> error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)
    in
    let (evd,ks,_) =
      List.fold_left
	(fun (evd,ks,m) b ->
	...
	(substn.us_sigma,[],List.length bs - 1) bs
    in
    let unilist2 f substn l l' =
      try List.fold_left2 f substn l l'
      with Invalid_argument "List.fold_left2" -> error_cannot_unify (fst curenvnb) substn.us_sigma (cM,cN)
    in
    let substn = unilist2 (fun s u1 u -> unirec_rec curenvnb pb b flags s u1 (substl ks u))
      (ex_from_subst (evd,ms,es) substn.us_delayed) us2 us in
    let substn = unilist2 (fun s u1 u -> unirec_rec curenvnb pb b flags s u1 (substl ks u))
      substn params1 params in
    let substn = unilist2 (unirec_rec curenvnb pb b flags) substn ts ts1 in
      unirec_rec curenvnb pb b flags substn c1 (applist (c,(List.rev ks)))

  in
  let evd = create_evar_defs top_sigma in
    if (if occur_meta_or_undefined_evar evd m || occur_meta_or_undefined_evar evd n then false 
      ...
      | Some flags ->
	  is_trans_fconv (conv_pb_of cv_pb) flags env top_sigma m n
      ...
      then error_cannot_unify env top_sigma (m, n) else false)
    then subst
    else
      let rec do_delayed = fun ((sigma, metasubst, evarsubst) as subst) delayed ->
        match delayed with
        | []            -> subst
        | ud :: delayed ->
            let left, right = ud.ud_unif_pb in
            let subst_left  = subst_meta_instances metasubst left
            and subst_right = subst_meta_instances metasubst right in
            let convertible =
              is_fconv
      ...
      in
      let state = ex_from_subst subst [] in
      let state = unirec_rec (env,0) cv_pb conv_at_top flags state m n in
