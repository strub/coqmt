...
let add_dp_bindings = fun mp ->
  let module DO = Decproc.OpCodes in

  let register  = fun opcode env ->
    match opcode with
...
and add_module mb env = 
  ...
  let env = Environ.shallow_add_module mp mb env in
    match mb.mod_type with
      | SEBstruct (sign) -> 
          let env = add_signature mp sign mb.mod_delta env in
          let env = add_retroknowledge mp mb.mod_retroknowledge env in
          let env = add_dp_bindings mp (mb.mod_dp) env in
            env
      | SEBfunctor _ -> env
